...
- **五渠道支付** — EasyPay 易支付聚合、支付宝官方、微信官方、Stripe、虎皮椒
...
通过虎皮椒收银台发起支付，下单接口使用 `POST /payment/do.html`，异步回调为表单 `POST`，成功处理后必须返回纯文本 `success`。

| 变量                   | 说明                         |
| ---------------------- | ---------------------------- |
| `XUNHU_PAY_APP_ID`     | 虎皮椒应用 APPID             |
| `XUNHU_PAY_APP_SECRET` | 虎皮椒应用密钥 APPSECRET     |
| `XUNHU_PAY_API_BASE`   | API 地址，默认官方生产地址   |
| `XUNHU_PAY_NOTIFY_URL` | 异步回调地址                 |
| `XUNHU_PAY_RETURN_URL` | 支付完成跳转地址（可选）     |

> 虎皮椒回调端点：`${NEXT_PUBLIC_APP_URL}/api/xunhupay/notify`
...
| `MAX_DAILY_AMOUNT_XUNHUPAY`      | 虎皮椒渠道每日全局限额（可选）           | 由提供商默认               |
...
| `POST` | `/api/xunhupay/notify` | 虎皮椒异步回调          |
